...
void updatePositions(unsigned long _dt){
    ...
    for(list<Element*>::iterator it=world.begin(); it != world.end(); it++){
        (*it)->pos.x += (*it)->spd.x*dt; (*it)->pos.y += (*it)->spd.y*dt;
        (*it)->spd.x += (*it)->acc.x*dt; (*it)->spd.y += (*it)->acc.y*dt;
    }
}
bool colideBorder(Element* e){
    ...
    bool col = false;
    if(e->pos.x + e->r > RESX){
        e->pos.x = RESX - e->r; e->spd.x = -BOUNCE_DESACC * abs(e->spd.x);
        col = true;
    }
    if(e->pos.y + e->r > RESY){
        e->pos.y = RESY - e->r; e->spd.y = -BOUNCE_DESACC *abs(e->spd.y);
        col = true;
    }
    if(e->pos.x - e->r < 0){
        e->pos.x = 0 + e->r; e->spd.x = BOUNCE_DESACC * abs(e->spd.x);
        col = true;
    }
    if(e->pos.y - e->r < TOP_BAR_HEI){
        e->pos.y = TOP_BAR_HEI+ e->r; e->spd.y = BOUNCE_DESACC * abs(e->spd.y);
        col = true;
    } 
    return col;
}
bool colideElements(Element* a,Element* b){
    if( dist(&(a->pos),&(b->pos)) < (a->r + b->r)){
        ...
        a->spd.x = -(a->spd.x - proj.x) + proj.x; a->spd.y = -(a->spd.y - proj.y) + proj.y;
        ...
        a->spd.x *= BOUNCE_DESACC; a->spd.y *= BOUNCE_DESACC;
...
void colision(){
    for(list<Element*>::iterator it=world.begin(); it != world.end(); it++){
        ...
        for(list<Element*>::iterator jt=world.begin(); jt != world.end(); jt++){
            if(it==jt) continue;
            if(colideElements((*it),(*jt))){ if((*it)->flags & KIL_COLI){ delete (*it); world.erase(it); } }
        }
    }
}
void gravity(){
...
void keyboardManagement(unsigned long _dt){
    ...
    float dt = ((float) _dt) / 1000.0f;
    if(state[SDL_SCANCODE_UP]){player1->spd.x += ENGINE_ACC*cos(player1->facing)*dt; player1->spd.y += ENGINE_ACC*sin(player1->facing)*dt;}
    if(state[SDL_SCANCODE_DOWN]){player1->spd.x -= ENGINE_ACC*cos(player1->facing)*dt; player1->spd.y -= ENGINE_ACC*sin(player1->facing)*dt;}
    if(state[SDL_SCANCODE_LEFT]){player1->facing -= FACING_SPEED*dt;}
    if(state[SDL_SCANCODE_RIGHT]){player1->facing += FACING_SPEED*dt;}
    ...
    if(player1->facing>2*PI) player1->facing -= 2*PI; else if(player1->facing<0) player1->facing += 2*PI;
    if(player2->facing>2*PI) player2->facing -= 2*PI; else if(player2->facing<0) player2->facing += 2*PI;
}
void mouseManagement(unsigned long _dt){
    ...
    if(state & SDL_BUTTON(SDL_BUTTON_RIGHT)){
            player2->spd.x += ENGINE_ACC*cos(player2->facing)*dt; player2->spd.y += ENGINE_ACC*sin(player2->facing)*dt;
    }
